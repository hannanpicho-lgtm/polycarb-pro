...
export function RecentComparisons({ currentPath, currentLabel, enabled }: RecentComparisonsProps) {
  ...
  useEffect(() => {
    ...
    const nextItems = [
      { path: currentPath, label: currentLabel, savedAt: new Date().toISOString() },
      ...parsed.filter((item) => item.path !== currentPath),
    ].slice(0, 5);

    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(nextItems));
    setItems(nextItems);
  }, [currentLabel, currentPath, enabled]);
  ...
  return (
    <div className="print:hidden rounded-xl border border-border bg-card p-5 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-brand-500 mb-1">Recent Comparisons</p>
