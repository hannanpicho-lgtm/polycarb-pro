...
interface CustomerHit { id: string; firstName: string; lastName: string; email: string; company?: string; }
...
const STATUS_COLOR: Record<string, string> = {
  pending: 'text-amber-600', confirmed: 'text-blue-600', processing: 'text-purple-600',
  shipped: 'text-sky-600', delivered: 'text-emerald-600', cancelled: 'text-slate-400',
  reviewed: 'text-blue-600', quoted: 'text-violet-600', accepted: 'text-emerald-600',
  rejected: 'text-red-500', converted: 'text-slate-400',
};
...
export function AdminSearch() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/search?q=${encodeURIComponent(debouncedQuery)}`)
      .then(r => r.json())
      .then(d => { setResults(d as SearchResults); setCursor(0); })
            ...
            >
              <div className="pointer-events-auto bg-white rounded-2xl shadow-2xl ring-1 ring-slate-200 overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100">
                  ...
                  />
                  <kbd onClick={() => setOpen(false)} className="cursor-pointer rounded border border-slate-200 bg-slate-100 px-1.5 py-0.5 text-[10px] font-semibold text-slate-500">
                    ESC
                  </kbd>
                </div>
                ...
                <div ref={listRef} className="max-h-[480px] overflow-y-auto">
                  {!query && (
                    <div className="px-4 py-8 text-center">
                      <p className="text-sm text-slate-400">Type to search across orders, quotes and customers</p>
                    </div>
                  )}
                  ...
                  {isEmpty && (
                    <div className="px-4 py-8 text-center">
                      <p className="text-sm text-slate-500">No results for <strong>&ldquo;{query}&rdquo;</strong></p>
                    </div>
                  )}
                  ...
                  {results && totalHits > 0 && (
                    <div className="py-2">
                      ...
                      {results.orders.length > 0 && (
                        <section>
                          <p className="px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-400">Orders</p>
                          {results.orders.map((o, i) => {
                            const globalIdx = i;
                            return (
                              <button key={o.id} onClick={() => handleSelect({ kind: 'order', data: o })}
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2">
                                    <span className="font-mono text-xs text-slate-500">{o.referenceId}</span>
                                    <span className={`text-[10px] font-semibold capitalize ${STATUS_COLOR[o.status] ?? ''}`}>{o.status}</span>
                                  </div>
                                  <p className="text-sm font-medium text-slate-800 truncate">
                                    {o.customerName}{o.customerCompany ? ` · ${o.customerCompany}` : ''}
                                  </p>
                                </div>
                      ...
                      {results.quotes.length > 0 && (
                        <section className="mt-1">
                          <p className="px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-400">Quotes</p>
                          {results.quotes.map((q, i) => {
                            const globalIdx = results.orders.length + i;
                            return (
                              <button key={q.id} onClick={() => handleSelect({ kind: 'quote', data: q })}
                                className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${cursor === globalIdx ? 'bg-brand-50' : 'hover:bg-slate-50'}`}
                                onMouseEnter={() => setCursor(globalIdx)}
                              >
                                <div className="w-7 h-7 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className={`text-[10px] font-semibold capitalize ${STATUS_COLOR[q.status] ?? ''}`}>{q.status}</span>
                                  </div>
                                  <p className="text-sm font-medium text-slate-800 truncate">
                      ...
                      {results.customers.length > 0 && (
                        <section className="mt-1">
                          <p className="px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-400">Customers</p>
                          {results.customers.map((c, i) => {
                            const globalIdx = results.orders.length + results.quotes.length + i;
                            return (
                              <button key={c.id} onClick={() => handleSelect({ kind: 'customer', data: c })}
                                className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${cursor === globalIdx ? 'bg-brand-50' : 'hover:bg-slate-50'}`}
                                onMouseEnter={() => setCursor(globalIdx)}
                              >
                                <div className="w-7 h-7 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0">
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-medium text-slate-800">{c.firstName} {c.lastName}{c.company ? ` · ${c.company}` : ''}</p>
                      ...
                      <div className="px-4 py-2 border-t border-slate-100 flex items-center justify-between mt-1">
                        <span className="text-[10px] text-slate-400">{totalHits} result{totalHits !== 1 ? 's' : ''}</span>
