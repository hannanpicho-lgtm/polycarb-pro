...
export function ParticleDots({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      {/* Metallic connector arc */}
      <path
        d="M22 84 C 55 36, 120 18, 158 46"
        stroke="url(#metalArc)"
        strokeWidth="1.1"
        strokeLinecap="round"
        fill="none"
            ...
            >
              ...
              <motion.circle
                ...
                transition={{
                  opacity: {
                    repeat: Infinity,
                    duration: 6.4 + sortedI * 0.09,
                    ease: 'easeInOut',
                    delay: dot.delay,
                  },
                  scale: {
                    repeat: Infinity,
                    duration: 7.0 + sortedI * 0.09,
                    ease: 'easeInOut',
                    delay: dot.delay * 0.8,
                  },
                }}
              ...
              <ellipse
                cx={dot.cx - dot.r * 0.24}
                cy={dot.cy - dot.r * 0.3}
                rx={dot.r * 0.3}
                ry={dot.r * 0.22}
                fill="#FFFFFF"
                fillOpacity={isDof ? 0.28 : 0.4}
                filter={isDof ? 'url(#dofBlur)' : undefined}
              />

              {/* Anisotropic highlight streak — thin horizontal band */}
              <ellipse
                cx={dot.cx - dot.r * 0.04}
                cy={dot.cy - dot.r * 0.46}
                rx={dot.r * 0.46}
                ry={dot.r * 0.07}
              ...
              <circle
                cx={dot.cx - dot.r * 0.08}
                cy={dot.cy - dot.r * 0.1}
                r={dot.r * 0.072}
                fill="#FFFFFF"
                fillOpacity={isDof ? 0.55 : 0.78}
              />

              {/* Shadow crescent — bottom-right */}
              <ellipse
                cx={dot.cx + dot.r * 0.32}
                cy={dot.cy + dot.r * 0.36}
                rx={dot.r * 0.24}
                ry={dot.r * 0.18}
                fill="#071220"
                fillOpacity={isDof ? 0.12 : 0.18}
                filter={isDof ? 'url(#dofBlur)' : undefined}
              />
            </motion.g>
          );
