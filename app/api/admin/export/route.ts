...
export async function GET(request: NextRequest) {
  ...
  if (type === 'orders') {
    ...
    const headers = ['Reference','Customer','Email','Company','Currency','Status','Payment',
                     'Subtotal','Shipping','Total','Region','Tracking','Notes',
                     'Confirmed','Shipped','Delivered','Created','Items'];
    const rows = (result.results ?? []).map((r: unknown) => {
      const row = r as Record<string, unknown>;
      return [
        row.referenceId, row.customerName, row.customerEmail, row.customerCompany,
        row.currency, row.status, row.paymentStatus,
        row.subtotal, row.shippingCost, row.total,
        row.shippingRegion, row.trackingNumber, row.adminNotes,
        row.confirmedAt, row.shippedAt, row.deliveredAt, row.createdAt,
        row.items,
      ];
    });
    ...
    const headers = ['Reference','Customer','Email','Company','Currency','Status','Source',
                     'Message','Admin Notes','Quoted Amount','Submitted','Responded','Created'];
    const rows = (result.results ?? []).map((r: unknown) => {
      const row = r as Record<string, unknown>;
      return [
        row.referenceId, row.customerName, row.customerEmail, row.customerCompany,
        row.currency, row.status, row.source, row.message, row.adminNotes,
        row.quotedAmount, row.submittedAt, row.respondedAt, row.createdAt,
      ];
    });
    csv = toCSV(headers, rows);
    ...
    const headers = ['First Name','Last Name','Email','Company','Phone','Region','Currency','Notes','Created'];
    const rows = (result.results ?? []).map((r: unknown) => {
      const row = r as Record<string, unknown>;
      return [row.firstName, row.lastName, row.email, row.company, row.phone,
              row.region, row.currency, row.notes, row.createdAt];
    });
    csv = toCSV(headers, rows);
    ...
    const headers = ['Name','Email','Company','Country','Status','Tier','Approved','Rejected','Applied'];
    const rows = (result.results ?? []).map((r: unknown) => {
      const row = r as Record<string, unknown>;
      return [row.fullName, row.email, row.companyName, row.country,
              row.status, row.discountTier, row.approvedAt, row.rejectedAt, row.createdAt];
    });
    csv = toCSV(headers, rows);
