...
export async function GET(request: NextRequest) {
  ...
  if (!token) return NextResponse.redirect(new URL('/distributor?error=missing_token', request.url));
  ...
  try {
    const row = await db.prepare(
      `SELECT id, email, expiresAt, usedAt FROM magic_links WHERE token = ? LIMIT 1`
    ...
    if (new Date(row.expiresAt) < new Date()) return NextResponse.redirect(new URL('/distributor?error=expired', request.url));
    ...
    ).bind(row.email).first<{ status: string }>();

    if (!dist) return NextResponse.redirect(new URL('/distributor?error=not_found', request.url));

    await db.prepare(`UPDATE magic_links SET usedAt = ? WHERE id = ?`).bind(new Date().toISOString(), row.id).run();
    ...
    response.cookies.set(DIST_COOKIE, cookieValue, {
      ...
      maxAge: DIST_COOKIE_MAX_AGE, path: '/',
    });
    return response;
  } catch (error) {
