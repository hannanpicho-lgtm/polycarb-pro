...
const STATUS_FLOW = [
  { key: 'pending',    label: 'Order Received',     desc: 'Your order has been submitted and is awaiting review.' },
  { key: 'confirmed',  label: 'Confirmed',           desc: 'Your order has been confirmed and is in our queue.' },
  { key: 'processing', label: 'Processing',          desc: 'Our team is preparing your order for dispatch.' },
...
const CANCELLED_STEP = { key: 'cancelled', label: 'Cancelled', desc: 'This order has been cancelled.' };
...
export async function GET(request: NextRequest) {
  ...
  if (!ref || !email) {
    return NextResponse.json(
      { error: 'Reference ID and email are required.' },
      { status: 400 }
    );
  }
  ...
  if (!order) {
    ...
    const quote = await db
      ...
      .first<{
        ...
        currency: string; status: string; createdAt: string; updatedAt: string;
      }>();
