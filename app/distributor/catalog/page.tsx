    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3">
          <span
            className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide ${tierInfo.color}`}
          >
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-0.5">
              Your net prices with {Math.round(tierInfo.discount * 100)}% {tierInfo.label} discount
              applied · {filtered.length} products
            </p>
          </div>
          <Link
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100">
          {filtered.map((p) => {
            ...
            return (
              <div
                key={p.slug}
                className="grid grid-cols-4 gap-2 items-center px-4 py-3.5 hover:bg-slate-50/50 transition-colors"
              >
                <div className="col-span-2 flex items-start gap-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span
                        className={`text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded ${catColor[cat]}`}
                      >
                        {CATEGORY_LABELS[cat]}
                      </span>
                      <span className="text-[10px] text-slate-400">
                        Min {p.minQty}kg · {p.leadTimeDays}d lead
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-brand-50 border border-brand-100 rounded-xl p-4 text-sm text-brand-800">
          ...
          <p className="text-brand-600 text-xs leading-relaxed">
            Submit a quote for a binding offer. Prices shown are ex-works in {currency} and exclude
            shipping. Tier pricing is subject to your annual volume commitment. Contact your account
            manager for volume discounts beyond {Math.round(tierInfo.discount * 100)}%.
          </p>
        </div>
      </div>
    </div>
