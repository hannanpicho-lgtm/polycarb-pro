...
function PendingState({ profile, email, onSignOut }: { profile: DistributorProfile; email: string; onSignOut: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center">
        ...
        <p className="text-slate-600 text-xs mb-6">Applied: {new Date(profile.createdAt).toLocaleDateString()}</p>
        <div className="flex gap-3 justify-center">
          <Link href="/contact" className="px-4 py-2 bg-brand-600 text-white text-sm font-medium rounded-lg hover:bg-brand-700 transition-colors">
            Contact us
          </Link>
          <button onClick={onSignOut} className="px-4 py-2 bg-white/5 border border-white/10 text-slate-400 text-sm font-medium rounded-lg hover:bg-white/10 transition-colors">
            Sign out
          </button>
        </div>
      </div>
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-400 text-sm">
        Unable to load. <Link href="/distributor" className="ml-1 text-brand-600 hover:underline">Sign in again</Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className={`hidden sm:inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide ${tierInfo.color}`}>
            {tierInfo.badge} {tierInfo.label} Partner
          </span>
          <button onClick={handleSignOut} className="px-3 py-1.5 text-xs text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
            Sign out
          </button>
        </div>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-gradient-to-r from-slate-900 to-brand-900 rounded-2xl p-6 text-white flex items-center justify-between gap-4 overflow-hidden relative">
          ...
          <div className="relative">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-brand-300 mb-1">Welcome back</p>
            <h1 className="text-2xl font-bold">{profile.companyName}</h1>
            <p className="text-slate-300 text-sm mt-0.5">{profile.fullName} · {profile.jobTitle ?? 'Partner'}</p>
          </div>
          <div className="relative text-right flex-shrink-0">
            <p className={`text-3xl font-black ${tierInfo.label === 'Gold' ? 'text-yellow-300' : tierInfo.label === 'Platinum' ? 'text-violet-300' : tierInfo.label === 'Silver' ? 'text-slate-200' : 'text-amber-300'}`}>
              {Math.round(tierInfo.discount * 100)}%
            </p>
            <p className="text-sm text-slate-300">off list price</p>
            <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[11px] font-bold mt-1 ${tierInfo.color}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: 'Browse Catalog',    href: '/distributor/catalog',   icon: '📦', sub: 'Net prices' },
            { label: 'New Quote',         href: '/distributor/quotes/new', icon: '📋', sub: 'Submit order request' },
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            ...
            {(['quotes', 'catalog'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-5 py-3.5 text-sm font-medium capitalize transition-colors ${
                  tab === t ? 'text-brand-600 border-b-2 border-brand-500' : 'text-slate-500 hover:text-slate-800'
                }`}>
            ...
            <div>
              {quotes.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <Link href="/distributor/quotes/new" className="inline-block mt-3 text-sm text-brand-600 hover:underline font-medium">
                    ...
                    {quotes.map(q => {
                      ...
                      return (
                        <tr key={q.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-slate-700 text-xs">{products.map(p => `${p.productName ?? '?'} × ${p.qty ?? '?'}kg`).join(', ')}</td>
                          <td className="px-4 py-3 font-semibold text-slate-900">{formatPrice(q.subtotalNet, q.currency as Currency)}</td>
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${QUOTE_STATUS_COLORS[q.status] ?? ''}`}>
                              {q.status}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-xs text-slate-400">{new Date(q.createdAt).toLocaleDateString()}</td>
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          {tab === 'catalog' && (
            <div className="p-5 space-y-3">
              <div className="flex items-center gap-3">
                <span className={`inline-flex items-center gap-1 rounded-full border px-3 py-1 text-sm font-bold ${tierInfo.color}`}>
                  {tierInfo.badge} {tierInfo.label} — {Math.round(tierInfo.discount * 100)}% off all list prices
                </span>
                <Link href="/distributor/catalog" className="text-sm text-brand-600 hover:underline ml-auto">
                  Full catalog with prices →
                </Link>
              </div>
              <p className="text-xs text-slate-500">
                All prices shown in the full catalog reflect your {tierInfo.label} tier discount applied to list prices.
                Prices are in USD by default. AUD available on request.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide mb-0.5">Company</p><p className="text-slate-800 font-medium">{profile.companyName}</p></div>
            <div><p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide mb-0.5">Contact</p><p className="text-slate-800">{profile.fullName}</p></div>
            <div><p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide mb-0.5">Email</p><p className="text-slate-800 break-all">{profile.email}</p></div>
            <div><p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide mb-0.5">Member since</p><p className="text-slate-800">{profile.approvedAt ? new Date(profile.approvedAt).toLocaleDateString() : '—'}</p></div>
          </div>
        </div>
      </div>
    </div>
