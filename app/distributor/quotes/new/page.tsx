...
export default function NewDistributorQuote() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([fetch('/api/distributor/me'), fetch('/api/catalog/prices')])
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
                ...
                >
                  ...
                  <div className="col-span-1">
                    ...
                    <p className="text-xs font-bold text-slate-900">
                      {lineTotal ? formatPrice(lineTotal, currency) : '—'}
                    </p>
                  </div>
          ...
          <section className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
            <h2 className="font-semibold text-slate-800">
              End Customer <span className="text-slate-400 font-normal text-xs">(optional)</span>
            </h2>
            <div className="grid grid-cols-2 gap-4">
              {(
                [
                  ['Name', endCustomerName, setEndCustomerName],
                  ['Company', endCustomerCompany, setEndCustomerCompany],
              ...
              <div>
                ...
                <input
                  type="text"
                  value={endCustomerCountry}
                  onChange={(e) => setEndCustomerCountry(e.target.value)}
                  className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-400/40"
                />
              </div>
              <div>
