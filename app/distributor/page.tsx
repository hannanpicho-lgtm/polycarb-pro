...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  not_found:     'No distributor account found for that email. Contact us if you believe this is an error.',
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex flex-col items-center gap-3">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-brand-500/30 to-brand-700/20 border border-brand-400/30 flex items-center justify-center shadow-lg shadow-brand-900/30">
              <svg className="w-8 h-8 text-brand-300" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            ...
            <div>
              ...
              <p className="text-sm text-brand-300/70 mt-0.5">Covestro Polycarbonates Partner Network</p>
            </div>
          </Link>
        </div>
          ...
          {sent ? (
            <div className="text-center py-4">
              ...
              <p className="text-slate-400 text-sm leading-relaxed">
                If <span className="text-white font-medium">{email}</span> is in our partner network, you&apos;ll receive a sign-in link within moments.
              </p>
              <button onClick={() => setSent(false)} className="mt-5 text-xs text-slate-500 hover:text-slate-300 transition-colors">
            ...
            <>
              ...
              <p className="text-slate-400 text-sm mb-6 leading-relaxed">
                Enter your registered partner email and we&apos;ll send you a secure, passwordless sign-in link.
              </p>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label htmlFor="email" className="block text-xs font-medium text-slate-300 mb-1.5">Partner email</label>
        ...
        <div className="mt-6 text-center space-y-2">
          <p className="text-xs text-slate-600">
            Not a partner yet?{' '}
            <Link href="/#distributor" className="text-slate-400 hover:text-white transition-colors">Apply to join our network</Link>
          </p>
          <p className="text-xs text-slate-700">
            Customer?{' '}
            <Link href="/portal" className="text-slate-500 hover:text-slate-300 transition-colors">Go to customer portal</Link>
          </p>
        </div>
      </div>
    </div>
...
export default function DistributorPage() {
  return <Suspense><DistributorLogin /></Suspense>;
}
