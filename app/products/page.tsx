...
function productsHrefWithCompare(params: SearchParams, compareSlugs: string[]) {
  ...
  compareSlugs.slice(0, 3).forEach((slug, i) => {
    query.set((['ca', 'cb', 'cc'] as const)[i]!, slug);
  });
...
function filterProducts(params: SearchParams) {
  ...
  if (params.industry && params.industry !== 'all') {
    filtered = filtered.filter((p) =>
      p.industries.includes(params.industry as ProductIndustry)
    );
  }
...
export default async function ProductsPage({
  ...
  const compareSlugs = Array.from(new Set([params.ca, params.cb, params.cc].filter(Boolean) as string[])).slice(0, 3);
  ...
  compareProducts.forEach((p, i) => comparePageParams.set((['a', 'b', 'c'] as const)[i]!, p.slug));
  const comparePageHref = comparePageParams.toString() ? `/products/compare?${comparePageParams.toString()}` : '/products/compare';
  const compareCsv = compareProducts.map((p) => p.slug).join(',');
    ...
    <>
      ...
      <div className="bg-steel-950 pt-28 pb-12">
        <div className="container mx-auto">
          <p className="text-brand-400 text-[11px] font-bold uppercase tracking-[0.15em] mb-2">Product Catalogue</p>
          ...
          <p className="text-white/60 text-base max-w-2xl leading-relaxed mb-6">
            {filtered.length} grade{filtered.length !== 1 ? 's' : ''} from world-leading producers. Filter by type, industry, or brand — then build an instant quote.
          </p>
          <Link
            href="/quote/builder"
      ...
      <div className="container mx-auto py-12">
        ...
        {activeFiltersCount > 0 && (
          <div className="flex items-center gap-2 flex-wrap mb-6">
            ...
            {params.sort && params.sort !== 'name-asc' && (
              <FilterChip label={params.sort === 'name-desc' ? 'Sort: Name Z-A' : params.sort === 'brand-asc' ? 'Sort: Brand' : 'Sort: In-Stock First'} />
            )}
          ...
          <div className="mb-6 rounded-lg border border-brand-300/60 bg-brand-50/60 p-3 sm:p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0">
                <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-brand-700 mb-1">Compare Shortlist</p>
                <p className="text-xs text-brand-800/80">
                  {compareProducts.length} of 3 selected. Add from cards below, then launch side-by-side comparison.
                </p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {compareProducts.map((p) => (
                    <span key={p.id} className="inline-flex items-center gap-1.5 rounded border border-brand-300 bg-background px-2 py-1 text-[11px] text-foreground">
                      <span className="truncate max-w-[170px]">{p.name}</span>
                      <Link
                        href={productsHrefWithCompare(params, compareSlugs.filter((slug) => slug !== p.slug))}
          ...
          {isCatalogView && filtered.length > 0 && (
            <div className="mb-12">
              <div className="text-center mb-8">
                <p className="text-xs font-bold uppercase tracking-widest text-brand-500 mb-2">Product Categories</p>
                <h3 className="text-2xl font-bold text-foreground mb-2">Complete Material Range</h3>
                <p className="text-muted-foreground max-w-xl mx-auto text-sm">
                  Whether you need precision sheets, engineered rods, advanced resins, or specialized compounds — we stock the complete spectrum.
                </p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {/* Sheets */}
                <Link href="?category=sheets" className="group relative overflow-hidden rounded-lg aspect-square bg-muted border border-border hover:border-brand-400 transition-all duration-300">
                  <Image
                ...
                {/* Rods */}
                <Link href="?category=rods" className="group relative overflow-hidden rounded-lg aspect-square bg-muted border border-border hover:border-brand-400 transition-all duration-300">
                  <Image
                ...
                {/* Resins */}
                <Link href="?category=resins" className="group relative overflow-hidden rounded-lg aspect-square bg-muted border border-border hover:border-brand-400 transition-all duration-300">
                  <Image
                ...
                {/* Specialty */}
                <Link href="?category=specialty" className="group relative overflow-hidden rounded-lg aspect-square bg-muted border border-border hover:border-brand-400 transition-all duration-300">
                  <Image
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                      {categoryProducts.map((product, idx) => (
                        <ProductCard
                          ...
                          addToCompareHref={productsHrefWithCompare(params, [...compareSlugs, product.slug])}
...
function ProductCard({
  ...
  return (
    <article
      className="group flex flex-col bg-card border border-border rounded-lg overflow-hidden hover:shadow-md hover:border-brand-200 dark:hover:border-brand-800 transition-all duration-300"
    >
      {/* Image */}
      ...
      <div className="flex flex-col flex-1 p-4">
        ...
        <div className="mt-4 pt-3 border-t border-border flex items-center gap-2">
          ...
          <Link
            href={isInCompare ? removeFromCompareHref : canAddToCompare ? addToCompareHref : comparePageHref}
          ...
          {product.datasheetUrl ? (
            <Button asChild size="sm" variant="ghost" className="text-xs h-7 px-2">
              <a {...getCatalogueLinkProps(product.datasheetUrl)} aria-label={`Download catalogue for ${product.name}`}>
          ...
          <Button asChild size="sm" className="text-xs h-7 px-3 bg-brand-500 hover:bg-brand-600 text-white border-0">
            <Link href={`/quote/builder?product=${product.slug}&source=products-catalog-card`}>Get Quote</Link>
          </Button>
        </div>
      </div>
    </article>
...
function EmptyState() {
  return (
    <div className="text-center py-20">
      <PackageSearch className="h-16 w-16 text-muted-foreground/30 mx-auto mb-4" aria-hidden="true" />
      <h3 className="text-xl font-bold text-foreground mb-2">No products match your filters</h3>
      <p className="text-muted-foreground text-sm mb-6 max-w-md mx-auto">
        Try broadening your search or removing filters. Can't find what you need? Our team can source specialty grades on request.
      </p>
