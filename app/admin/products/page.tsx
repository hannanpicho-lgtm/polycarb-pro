...
const CATEGORY_MAP: Record<string, string> = {
  'solid-sheet': 'Sheets', 'twinwall': 'Sheets', 'multiwall': 'Sheets', 'solar-control': 'Sheets',
  'rod': 'Rods & Profiles', 'tube': 'Rods & Profiles', 'plate': 'Rods & Profiles',
  'resin': 'Resins', 'compound': 'Resins',
};
function guessCategory(slug: string): string {
  ...
  if (slug.includes('resin') || slug.includes('lexan-940') || slug.includes('gp1000') ||
      slug.includes('gf30') || slug.includes('ep5030') || slug.includes('exl') ||
      slug.includes('l1225') || slug.includes('h3000')) return 'Resins';
  if (slug.includes('rod') || slug.includes('tube') || slug.includes('plate')) return 'Rods & Profiles';
  if (slug.includes('sheet') || slug.includes('thermoclear') || slug.includes('panlite-tw') ||
      slug.includes('iupilon-pc-sheet') || slug.includes('lupoy-pc-sheet') ||
      slug.includes('calibre-solid') || slug.includes('makrolon-2')) return 'Sheets';
  return 'Specialty';
}
...
export default function AdminProductsPage() {
  ...
  async function handleToggle(slug: string, field: 'isActive' | 'featured', current: boolean) {
    ...
    try {
      ...
      setProducts((prev) => prev.map((p) => p.slug === slug ? { ...p, [field]: !current, inDB: true } : p));
    ...
    <div className="p-6 max-w-[1400px]">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage prices, availability and lead times without a code deploy.</p>
        </div>
          ...
          {categories.map((c) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                filterCategory === c ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
              }`}
            >
              {c}
            </button>
          ...
          {Object.entries(grouped).map(([category, items]) => (
            <div key={category} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 bg-slate-50">
                <div className="flex items-center gap-2">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${CATEGORY_COLORS[category] ?? 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                    {category}
                  </span>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="text-left px-4 py-2 text-xs font-semibold text-slate-500 w-[300px]">Product</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-slate-500">USD / kg</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-slate-500">AUD / kg</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-slate-500">Min Qty</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-slate-500">Lead time</th>
                    <th className="text-center px-3 py-2 text-xs font-semibold text-slate-500">Active</th>
                    <th className="text-center px-3 py-2 text-xs font-semibold text-slate-500">Featured</th>
                ...
                <tbody>
                  {items.map((product) => {
                    ...
                    const audEffective = product.unitPriceAUD != null
                      ...
                      <>
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-start gap-2">
                              <div>
                                ...
                                {product.adminNotes && (
                                  <p className="text-[11px] text-amber-600 mt-0.5 italic">{product.adminNotes}</p>
                                )}
                              </div>
                              {!product.inDB && (
                                <span className="flex-shrink-0 text-[9px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-400 font-mono mt-0.5">static</span>
                              )}
                            </div>
                          </td>
                          ...
                          <td className="px-3 py-3 text-center">
                            <button
                              onClick={() => handleToggle(product.slug, 'isActive', product.isActive)}
                              disabled={saving}
                          ...
                          <td className="px-3 py-3 text-center">
                            <button
                              onClick={() => handleToggle(product.slug, 'featured', product.featured)}
                              disabled={saving}
                          ...
                          <td className="px-3 py-3 text-right">
                            <button
                              onClick={() => isEditing ? setEditingSlug(null) : startEdit(product)}
                          ...
                          <tr ref={editRef} className="border-b border-brand-100 bg-brand-50/30">
                            <td colSpan={8} className="px-4 py-4">
                              <div className="flex items-end gap-4 flex-wrap">
                                <div>
                                  <label className="block text-xs font-semibold text-slate-600 mb-1">USD Price / kg</label>
                                  <div className="relative">
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => setEditState((s) => ({ ...s, unitPriceUSD: e.target.value }))}
                                ...
                                <div>
                                  <label className="block text-xs font-semibold text-slate-600 mb-1">AUD Price / kg <span className="text-slate-400 font-normal">(leave blank = auto)</span></label>
                                  <div className="relative">
                                    <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-sm">A$</span>
                                    <input
                                      type="number"
                                      step="0.01"
                                ...
                                <div>
                                  <label className="block text-xs font-semibold text-slate-600 mb-1">Min Qty (kg)</label>
                                  <input
                                    type="number"
                                    step="1"
                                    value={editState.minQty}
                                    onChange={(e) => setEditState((s) => ({ ...s, minQty: e.target.value }))}
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setEditState((s) => ({ ...s, leadTimeDays: e.target.value }))}
                                    className="px-3 py-1.5 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-400 w-24 font-mono"
                                  />
                                </div>
                                ...
                                <div className="flex-1 min-w-[180px]">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setEditState((s) => ({ ...s, adminNotes: e.target.value }))}
