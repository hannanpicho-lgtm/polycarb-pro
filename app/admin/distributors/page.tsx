...
export default function DistributorsPage() {
  ...
  const fetchApps = useCallback(async () => {
    ...
    try {
      ...
      submissions.forEach((a) => {
        tierMap[a.id] = a.discountTier || 'bronze';
      });
  ...
  function volumeScore(v: string) {
    return VOLUME_SCORE[v] ?? 0;
  }
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            {total} total · Approve to activate portal access
          </p>
        </div>
        {/* Stats row */}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((app) => {
                ...
                const tier =
                  tierSelections[app.id] ?? ((app.discountTier || 'bronze') as DiscountTier);
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <div
                              key={i}
                              className={`w-2 h-2 rounded-full ${i < score ? 'bg-brand-500' : 'bg-slate-200'}`}
                            />
                          ))}
                        </div>
                        <p className="text-[10px] text-slate-400 mt-0.5">
                          {app.estimatedAnnualVolume}
                        </p>
                      </td>
                      <td className="px-4 py-3">
                        {status === 'approved' ? (
                          <span
                            className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${tierInfo.color}`}
                          >
                      ...
                      <tr key={`${app.id}-detail`} className="bg-slate-50/60">
                        <td colSpan={7} className="px-6 py-6">
                          <div className="grid grid-cols-3 gap-4 mb-5">
                            ...
                            <Field
                              label="Countries"
                              value={parseCountries(app.countries)}
                              className="col-span-2"
                            />
                            <Field label="Annual Volume" value={app.estimatedAnnualVolume} />
                            {app.message && (
                              <Field label="Message" value={app.message} className="col-span-3" />
                            )}
                          </div>
                          ...
                          {status !== 'approved' && status !== 'rejected' && (
                            <div className="border-t border-slate-200 pt-4 space-y-4">
                                  ...
                                  {TIERS.map((t) => {
                                    ...
                                    return (
                                      <button
                                        key={t}
                                        onClick={() =>
                                          setTierSelections((prev) => ({ ...prev, [app.id]: t }))
                                        }
                              ...
                              <div>
                                ...
                                <textarea
                                  rows={2}
                                  value={notesInputs[app.id] ?? app.internalNotes ?? ''}
                                  onChange={(e) =>
                                    setNotesInputs((prev) => ({
                                      ...prev,
                              ...
                              <div className="flex flex-wrap gap-2 items-start">
                                {status === 'pending' && (
                                  <button
                                    onClick={() => updateApp(app.id, 'review')}
                                ...
                                <button
                                  onClick={() => updateApp(app.id, 'approve')}
                                  disabled={updatingId === app.id}
                                ...
                                >
                                  {updatingId === app.id
                                    ? 'Approving…'
                                    : `✓ Approve as ${TIER_CONFIG[tier].label}`}
                                </button>
                                <div className="flex gap-2 items-center ml-auto">
                                  ...
                                  />
                                  <button
                                    onClick={() => updateApp(app.id, 'reject')}
                                    disabled={updatingId === app.id}
                          ...
                          {status === 'approved' && (
                            <div className="border-t border-slate-200 pt-4 flex items-center gap-4">
                              <span
                                className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-sm font-semibold ${TIER_CONFIG[app.discountTier || 'bronze'].color}`}
                              >
                                {TIER_CONFIG[app.discountTier || 'bronze'].badge}{' '}
                                {TIER_CONFIG[app.discountTier || 'bronze'].label} Partner
                                &nbsp;·&nbsp;{' '}
                                {Math.round(
                                  TIER_CONFIG[app.discountTier || 'bronze'].discount * 100
                                )}
                                % off list
                              </span>
                              {app.approvedAt && (
                              ...
                              <a
                                href={`mailto:${app.email}?subject=Your Covestro PC Distributor Account`}
