...
import { productPrices, shippingRegions, convertPrice, formatPrice, calcShipping } from '@/lib/pricing';
...
function NewOrderContent() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-6">
        <button onClick={() => router.back()} className="text-sm text-slate-500 hover:text-slate-800 mb-2 flex items-center gap-1">
          ← Back
        </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Email *" type="email" required value={customerEmail} onChange={setCustomerEmail} />
        ...
        <section className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          ...
          {items.length === 0 && (
            <p className="text-sm text-slate-400 text-center py-4">No products added yet — click "Add line"</p>
          )}
          {items.map((item, i) => (
            <div key={i} className="grid grid-cols-12 gap-2 items-end border-b border-slate-100 pb-4">
              ...
              <div className="col-span-3">
                <label className="block text-xs font-medium text-slate-600 mb-1">Unit price ({currency})</label>
                <input type="number" min={0} step="0.01" value={convertPrice(item.unitPrice, currency).toFixed(2)}
                  onChange={e => {
              ...
              <div className="col-span-1">
                ...
                <p className="text-xs font-semibold text-slate-700">{formatPrice(item.qty * convertPrice(item.unitPrice, currency), currency)}</p>
              </div>
        ...
        <section className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-slate-900 py-2">{formatPrice(shippingCost, currency)}</p>
            </div>
            <div className="col-span-2">
...
export default function NewOrderPage() {
  return <Suspense><NewOrderContent /></Suspense>;
}
