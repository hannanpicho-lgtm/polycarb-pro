...
function StatCard({
  ...
  return (
    <div
      className={`bg-white border rounded-xl px-5 py-4 ${color ? `border-t-2 ${color}` : 'border-slate-200'}`}
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Download CSV files for accounting, CRM, or analysis
          </p>
        </div>
      </div>
...
export default function ReportsPage() {
  ...
  const topProductMax = data?.topProducts.length
    ? Math.max(...data.topProducts.map((p) => p.totalRevenue), 1)
    : 1;
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            Revenue, pipeline, product and partner analytics
          </p>
        </div>
        <button
          onClick={fetchData}
      ...
      {loading ? (
        ...
        <>
          ...
          <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard
              label="Collected"
              value={formatPrice(totalCollected, 'USD')}
              sub={`${totalRevenue > 0 ? Math.round((totalCollected / totalRevenue) * 100) : 0}% collection rate`}
            ...
            <StatCard
              ...
              value={`${data.quoteConversionRate}%`}
              sub={`${data.convertedQuotes} of ${data.totalQuotes} quotes`}
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-4 text-xs">
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded bg-brand-500 inline-block" />
                  Revenue
                </span>
                <span className="flex items-center gap-1.5">
                ...
                return (
                  <div key={m.month} className="flex-1 flex flex-col items-center gap-1 group">
                    <div className="relative w-full flex items-end gap-0.5 h-32">
                      <div
                        className="flex-1 bg-brand-100 rounded-t transition-all duration-500 group-hover:bg-brand-200"
                    ...
                    {m.orderCount > 0 && (
                      <span className="text-[9px] text-slate-300">{m.orderCount}</span>
                    )}
                  </div>
                );
              })}
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div key={s.status} className="space-y-1">
                        <div className="flex justify-between items-center text-xs">
                          ...
                          <span className="text-slate-500">
                            {s.count} orders · {s.value ? formatPrice(s.value, 'USD') : '—'}
                          </span>
                        </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-full ${data.quoteConversionRate >= 20 ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}
                >
          ...
          {/* ── Top products ── */}
          <section className="bg-white border border-slate-200 rounded-2xl p-6">
            <h2 className="text-sm font-bold uppercase tracking-wide text-slate-600 mb-5">
              Top Products by Revenue
            </h2>
            {data.topProducts.length === 0 ? (
              <p className="text-xs text-slate-400">
                No order items yet — add orders to see product analytics.
              </p>
            ) : (
                  ...
                  <div key={p.productSlug} className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <p className="text-sm font-bold text-slate-900 ml-2 flex-shrink-0">
                          {formatPrice(p.totalRevenue, 'USD')}
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.max(2, (p.totalRevenue / topProductMax) * 100)}%`,
                            }}
                          />
                        </div>
                        <span className="text-[11px] text-slate-400 flex-shrink-0">
                          {p.totalQty} kg · {p.orderCount} order{p.orderCount !== 1 ? 's' : ''}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6">
              ...
              {data.paymentBreakdown.length === 0 ? (
                ...
                <BarChart
                  data={data.paymentBreakdown.map((r) => ({
                    label: r.status,
                    value: r.value ?? 0,
                  }))}
              ...
              {(data.distributors.distRevenue ?? 0) > 0 && (
                <div className="mt-4 border-t border-slate-100 pt-4">
                  ...
                  <p className="text-lg font-black text-brand-600 mt-0.5">
                    {formatPrice(data.distributors.distRevenue, 'USD')}
                  </p>
                </div>
              )}
            </div>
