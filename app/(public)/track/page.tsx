      ...
      <div className="max-w-3xl mx-auto px-4 py-12 space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
              <div className="bg-gradient-to-r from-brand-600 to-brand-700 px-6 py-5">
                  ...
                  <div className="text-right">
                    {result.type === 'order' && result.total !== undefined && (
                      <p className="text-white text-xl font-black">
                        {fmt(result.total, result.currency)}
                      </p>
                    )}
                    <p className="text-brand-200 text-xs mt-0.5">
                      Placed {fmtDate(result.createdAt)}
                    </p>
                  </div>
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  {result.paymentStatus && (
                    <span
                      className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${PAY_BADGE[result.paymentStatus] ?? 'bg-white/20 text-white'}`}
                    >
                      {result.paymentStatus}
                    </span>
                  )}
              ...
              <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
                ...
                <p className="text-sm text-slate-700 font-medium">
                  {result.customerName}
                  {result.customerCompany ? ` · ${result.customerCompany}` : ''}
                </p>
              </div>
              ...
              {result.type === 'quote' && result.quoteStatusMap && (
                <div className="px-6 py-5">
                  <div className="flex items-start gap-3 rounded-xl bg-slate-50 border border-slate-200 px-4 py-4">
                    <span
                      className={`mt-0.5 inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${QUOTE_STATUS_BADGE[result.status] ?? 'bg-slate-100 text-slate-600'}`}
                    >
                      {result.status}
                    </span>
                ...
                <div className="px-6 py-6">
                  ...
                  <ol className="relative">
                    {result.timeline.map((step, i) => (
                      <li
                        key={step.key}
                        className={`relative flex gap-4 ${i < result.timeline!.length - 1 ? 'pb-6' : ''}`}
                      >
                        {/* Connector line */}
                        {i < result.timeline!.length - 1 && (
                          <div
                            className={`absolute left-[13px] top-7 bottom-0 w-0.5 ${step.done ? 'bg-brand-400' : 'bg-slate-200'}`}
                          />
                        )}
                        {/* Dot */}
                        <div
                        ...
                        <div className="pt-0.5 min-w-0">
                          <p
                            className={`text-sm font-semibold ${step.active ? 'text-brand-600' : step.done ? 'text-slate-800' : 'text-slate-400'}`}
                          >
                            {step.label}
                            {step.active && (
                              <span className="ml-2 inline-flex px-1.5 py-0.5 rounded bg-brand-100 text-brand-700 text-[10px] font-bold uppercase">
                                Current
                              </span>
                            )}
                          </p>
                        ...
                        >
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-slate-400">
                              {item.qty}
                              {item.unit} × {fmt(item.unitPrice, result.currency)}
                            </p>
                          </div>
        ...
        {!result && (
          <div className="text-center text-slate-500 text-xs space-y-1">
            ...
            <p>
              It looks like{' '}
              <code className="bg-white/10 px-1 rounded text-slate-300">ORD-XXXXXXXX</code> for
              orders or <code className="bg-white/10 px-1 rounded text-slate-300">QT-XXXXX</code>{' '}
              for quotes.
            </p>
          </div>
        )}
