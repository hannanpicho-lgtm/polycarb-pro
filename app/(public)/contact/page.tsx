...
import { getSiteOfficeGoogleMapsUrl, siteConfig } from '@/lib/site-config';
import { resolvePublicLeadProductContext } from '@/lib/public-catalog';
import { COMPARE_QUERY_PARAM_KEYS } from '@/lib/public-catalog-constants';

export const metadata: Metadata = {
  title: 'Contact Us',
  description: `Get in touch with ${siteConfig.company.name} for quotes, technical support, and material consultation. We respond within 1 business day.`,
};

const officeMapsUrl = getSiteOfficeGoogleMapsUrl();
...
const enquiryTypes = [
  {
    ...
    desc: 'Get pricing for specific grades and volumes.',
  },
  { icon: FileText, label: 'Download Datasheets', desc: 'Access full TDS and MSDS documents.' },
  {
    icon: Users,
    label: 'Technical Consultation',
    desc: 'Talk to an applications engineer about your project.',
  },
  {
    icon: Truck,
    label: 'Distributor partnership',
    desc: 'Apply for tiered pricing and partner support.',
    href: '/distributors?source=contact-enquiry',
  },
];
...
export default async function ContactPage({
  ...
  const fromParamOrCookie = (paramValue: string | undefined, cookieName: string) =>
    paramValue ?? cookieStore.get(cookieName)?.value;
  ...
  const { product, compareProducts } = await resolvePublicLeadProductContext({
    productSlug: params.product,
    compareCsv: params.compare,
  });
  ...
  const sourcePath =
    compareProducts.length > 0
      ? `/products/compare?${compareProducts
          .map((item, i) => `${COMPARE_QUERY_PARAM_KEYS[i]!}=${item.slug}`)
  ...
  const hiddenContext = {
    leadSource:
      params.source ??
      (compareProducts.length > 0 ? 'compare-page' : product ? 'product-page' : 'contact-page'),
  ...
  const contextualProducts =
    compareProducts.length > 0 ? compareProducts : product ? [product] : [];
      ...
      <div className="bg-steel-950 pt-28 pb-14">
        <div className="container mx-auto">
          ...
          <p className="text-white/60 text-base max-w-xl leading-relaxed">
            Quotes, datasheets, or material selection guidance — we'll connect you with the right
            engineer.
          </p>
                ...
                <div className="mb-5 rounded-lg border border-brand-300/60 bg-brand-50/70 p-4">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {contextualProducts.map((item) => (
                      <span
                        key={item.slug}
                        className="inline-flex items-center rounded border border-brand-300 bg-background px-2.5 py-1 text-xs text-foreground"
                      >
                        {item.name}
                      </span>
                    ))}
