...
import { COMPARE_QUERY_PARAM_KEYS, PUBLIC_COMPARE_MAX_SLUGS } from '@/lib/public-catalog-constants';
...
function removeSlugHref(slugs: string[], remove: string, showOnlyDiff: boolean) {
  ...
  remaining.forEach((s, i) => params.set(COMPARE_QUERY_PARAM_KEYS[i]!, s));
...
function addSlugHref(existingSlugs: string[], newSlug: string, showOnlyDiff: boolean) {
  if (existingSlugs.includes(newSlug) || existingSlugs.length >= PUBLIC_COMPARE_MAX_SLUGS)
    return null;
  const next = [...existingSlugs, newSlug];
  const params = new URLSearchParams();
  next.forEach((s, i) => params.set(COMPARE_QUERY_PARAM_KEYS[i]!, s));
  if (showOnlyDiff) params.set('onlydiff', '1');
  return `/products/compare?${params.toString()}`;
}
...
export default async function ComparePage({ searchParams }: Props) {
  ...
  const invalidSlugs = uniqueRequestedSlugs.filter(
    (slug) => !getProductBySlug(slug) || hidden.has(slug)
  );
  const normalizedSlugs = uniqueRequestedSlugs
    .filter((slug) => !invalidSlugs.includes(slug))
  ...
  const emptySlots = Math.max(0, PUBLIC_COMPARE_MAX_SLUGS - selectedProducts.length);

  // Products available to add (not already selected)
  const addableProducts = filterByPublicProductVisibility(products, hidden).filter(
    (p) => !selectedProducts.some((sp) => sp.slug === p.slug)
  );

  const selectedParams = new URLSearchParams();
  selectedProducts.forEach((p, i) => selectedParams.set(COMPARE_QUERY_PARAM_KEYS[i]!, p.slug));
  ...
  if (selectedProducts.length > 0)
    quoteParams.set('compare', selectedProducts.map((p) => p.slug).join(','));
  ...
  const toggleDiffPath = toggleDiffParams.toString()
    ? `/products/compare?${toggleDiffParams.toString()}`
    : '/products/compare';
  ...
  const featuresDiffers = hasDifferences(
    selectedProducts.map((p) => p.features.slice(0, 4).join('|'))
  );
    ...
    <>
      ...
      <div className="bg-steel-950 pt-28 pb-12">
        <div className="container mx-auto">
          ...
          <p className="text-white/65 text-base max-w-2xl leading-relaxed">
            {`Select up to ${PUBLIC_COMPARE_MAX_SLUGS} polycarbonate grades to compare specifications side by side.`}
          </p>
        </div>
      </div>
      ...
      <div className="bg-background py-12">
        <div className="container mx-auto">
          ...
          {selectedProducts.length === 0 && (
            <div className="text-center py-20">
              <p className="text-muted-foreground text-sm mb-4">
                No products selected. Choose grades from the catalog to compare.
              </p>
            ...
            <>
              ...
              {showOnlyDiff && visibleInfoRowsCount === 0 ? (
                <div className="mb-4 rounded-lg border border-emerald-300/60 bg-emerald-50 px-4 py-3 text-xs font-medium text-emerald-800 print:hidden">
                  No differences found across selected rows. You can still use this page to quote or
                  review details.
                </div>
              ) : null}
                  ...
                  <thead>
                    <tr>
                      ...
                      {selectedProducts.map((p) => (
                        <th
                          key={p.id}
                          className="px-4 py-3 text-left bg-muted/40 border-l border-border"
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                              ...
                              <span
                                className={`inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded ${p.inStock ? 'bg-emerald-500/20 text-emerald-600' : 'bg-amber-500/20 text-amber-600'}`}
                              >
                      ...
                      {Array.from({ length: emptySlots }).map((_, i) => (
                        <th
                          key={`empty-${i}`}
                          className="px-4 py-3 bg-muted/20 border-l border-dashed border-border text-center"
                        >
                    ...
                    <tr>
                      ...
                      {Array.from({ length: emptySlots }).map((_, i) => (
                        <td
                          key={`img-empty-${i}`}
                          className="px-4 py-3 bg-background border-l border-dashed border-border"
                        />
                      ))}
                    </tr>
                  </thead>
                  ...
                  <tbody>
                      ...
                      <tr key={key} className={rowIdx % 2 === 0 ? 'bg-background' : 'bg-muted/30'}>
                        ...
                        {selectedProducts.map((p) => {
                          ...
                          return (
                            <td
                              key={p.id}
                              className="px-4 py-3 border-l border-border font-mono text-xs text-foreground"
                            >
                        ...
                        {Array.from({ length: emptySlots }).map((_, i) => (
                          <td
                            key={`spec-empty-${i}`}
                            className="px-4 py-3 border-l border-dashed border-border"
                          />
                        ))}
                      </tr>
                    ))}
                    ...
                    {!showOnlyDiff || categoryDiffers ? (
                      <tr className="bg-background">
                        ...
                        {selectedProducts.map((p) => (
                          <td
                            key={p.id}
                            className="px-4 py-3 border-l border-border text-xs text-foreground capitalize"
                          >
                            {p.category}
                          </td>
                        ))}
                            ...
                            <div className="flex flex-wrap gap-1">
                              {p.certifications.length > 0 ? (
                                p.certifications.map((c) => (
                                  <span
                                    key={c}
                                    className="text-[10px] border border-border text-muted-foreground px-1.5 py-0.5 rounded font-mono"
                                  >
                                    {c}
                                  </span>
                                ))
                          ...
                          <td key={p.id} className="px-4 py-3 border-l border-border">
                            <div className="flex flex-wrap gap-1">
                              {p.industries.map((ind) => (
                                <span
                                  key={ind}
                                  className="text-[10px] bg-brand-50 dark:bg-brand-950/30 text-brand-700 dark:text-brand-400 px-1.5 py-0.5 rounded capitalize"
                                >
                                  {ind}
                                </span>
                              ))}
                    ...
                    <tr className="bg-background border-t border-border">
                        ...
                        <td key={p.id} className="px-4 py-4 border-l border-border">
                          <div className="flex flex-col gap-2">
                            <Button
                              asChild
                              size="sm"
                              className="bg-brand-500 hover:bg-brand-600 text-white w-full text-xs"
                            >
                              <Link
                                href={`/quote?product=${p.slug}&compare=${selectedProducts.map((sp) => sp.slug).join(',')}${showOnlyDiff ? '&onlydiff=1' : ''}&source=compare-table-row`}
                              >
                                <MessageSquare className="h-3.5 w-3.5 mr-1.5" />
                                Request Quote
                              </Link>
                      ...
                      {Array.from({ length: emptySlots }).map((_, i) => (
                        <td
                          key={`cta-empty-${i}`}
                          className="px-4 py-4 border-l border-dashed border-border"
                        />
                      ))}
                    </tr>
                  </tbody>
          ...
          {/* Add more products */}
          {selectedProducts.length < PUBLIC_COMPARE_MAX_SLUGS && addableProducts.length > 0 && (
            <div className="mt-10 print:hidden">
              <p className="text-sm font-semibold text-foreground mb-4">
                Add a grade to compare ({PUBLIC_COMPARE_MAX_SLUGS - selectedProducts.length} slot
                {PUBLIC_COMPARE_MAX_SLUGS - selectedProducts.length !== 1 ? 's' : ''} remaining)
              </p>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {addableProducts.slice(0, 12).map((p) => {
                  ...
                  return (
                    <article
                      key={p.id}
                      className="border border-border rounded-lg p-3 bg-card flex items-center gap-3 hover:border-brand-300 transition-colors"
                    >
              ...
              {addableProducts.length > 12 && (
                <Link
                  href="/products"
                  className="mt-4 inline-flex text-xs font-semibold text-brand-600 hover:text-brand-700 transition-colors"
                >
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <h2 className="text-lg font-bold text-foreground font-display">
                    Send your compared grades to sales in one step
                  </h2>
                  <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
                    We will prefill the contact form with your selected grades so your team can
                    request pricing, MOQ, and lead-time guidance without retyping product names.
                  </p>
                </div>
                <Button asChild className="bg-brand-500 hover:bg-brand-600 text-white">
                  <Link href={compareQuoteHref}>
