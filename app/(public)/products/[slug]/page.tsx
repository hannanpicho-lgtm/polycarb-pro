...
import {
  getDatasheetsForProduct,
  getProductBySlug,
  getRelatedProducts,
  products,
} from '@/lib/data';
import {
  filterByPublicProductVisibility,
  notFoundIfProductHiddenInPublicCatalog,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  await notFoundIfProductHiddenInPublicCatalog(slug);
...
export default async function ProductDetailPage({ params }: Props) {
  const { slug } = await params;
  const hidden = await notFoundIfProductHiddenInPublicCatalog(slug);
  const product = getProductBySlug(slug);

  if (!product) notFound();

  const relatedDatasheets = getDatasheetsForProduct(product).filter(
    (doc) => doc.url !== product.datasheetUrl
  );
  const similarProducts = filterByPublicProductVisibility(getRelatedProducts(product, 4), hidden);
      ...
      <div className="bg-steel-950 pt-24 pb-0">
        <div className="container mx-auto py-4">
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-white/50">
            ...
            <span>/</span>
            <CompareAwareLink href="/products" className="hover:text-white transition-colors">
              Products
            </CompareAwareLink>
            <span>/</span>
              ...
              <div className="space-y-2 mb-8">
                {product.features.map((f) => (
                  <div key={f} className="flex items-start gap-2.5">
                    <CheckCircle2
                      className="h-4 w-4 text-brand-400 mt-0.5 flex-shrink-0"
                      aria-hidden="true"
                    />
        ...
        <div className="container mx-auto">
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <h2 className="text-xl font-bold mb-4 text-foreground font-display">
                Technical Specifications
              </h2>
              ...
              {/* Full description */}
              <h2 className="text-xl font-bold mt-8 mb-3 text-foreground font-display">
                Product Description
              </h2>
              ...
              <div className="flex flex-wrap gap-2">
                {product.applications.map((app) => (
                  <span
                    key={app}
                    className="bg-muted text-muted-foreground text-xs px-3 py-1.5 rounded"
                  >
                    {app}
                  </span>
                ))}
            ...
            <div className="space-y-5">
              ...
              <div className="bg-brand-500 rounded-lg p-6 text-white">
                ...
                <p className="text-white/80 text-sm mb-4">
                  Tell us your volume requirements and we'll respond within 1 business day with
                  competitive pricing.
                </p>
                <Button asChild variant="white" size="sm" className="w-full font-bold">
                  <Link href={`/quote?product=${product.slug}&source=product-detail-sidebar`}>
                    Request Quote
                  </Link>
                </Button>
              </div>
              ...
              <div className="bg-muted/50 border border-border rounded-lg p-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {product.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[11px] bg-background border border-border text-muted-foreground px-2 py-0.5 rounded font-mono"
                    >
                      #{tag}
                    </span>
                  ))}
              ...
              {relatedDatasheets.length > 0 ? (
                <div className="bg-muted/50 border border-border rounded-lg p-5">
                  <h3 className="font-bold text-sm text-foreground mb-3 font-display">
          ...
          {similarProducts.length > 0 ? (
            <section className="mt-12 pt-10 border-t border-border">
              <div className="flex items-end justify-between mb-6">
                <div>
                  <p className="text-brand-500 text-[11px] font-bold uppercase tracking-[0.15em] mb-1">
                    Keep Exploring
                  </p>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {similarProducts.map((p) => (
                  <article
                    key={p.id}
                    className="group border border-border rounded-lg bg-card overflow-hidden hover:shadow-md hover:border-brand-200 transition-all duration-300"
                  >
                    ...
                    <div className="p-4">
                      ...
                      <h3 className="text-sm font-bold text-foreground leading-snug mb-2 group-hover:text-brand-600 transition-colors line-clamp-2">
                        {p.name}
                      </h3>
