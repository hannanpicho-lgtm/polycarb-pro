...
export const metadata: Metadata = {
  ...
  description:
    'Submit a formal quote request for polycarbonate grades with project details and volume requirements.',
};
...
export default async function QuotePage({
  ...
  const sourcePath =
    compareProducts.length > 0
      ? `/products/compare?${compareProducts.map((item, i) => `${COMPARE_QUERY_PARAM_KEYS[i]!}=${item.slug}`).join('&')}${params.onlydiff === '1' ? '&onlydiff=1' : ''}`
      : product
  ...
  const hiddenContext = {
    leadSource:
      params.source ??
      (compareProducts.length > 0 ? 'compare-page' : product ? 'product-page' : 'quote-page'),
      ...
      <div className="bg-steel-950 pt-28 pb-14">
        <div className="container mx-auto">
          ...
          <p className="text-white/60 text-base max-w-2xl leading-relaxed mb-6">
            Share grade selection, quantities, and destination details. Our team returns pricing and
              ...
              {initialProduct ? (
                <div className="mb-5 rounded-lg border border-brand-300/60 bg-brand-50/70 p-4">
                  ...
                  <p className="text-sm text-brand-900/85 mb-3">
                    Your selected products have been preloaded into the request form.
                  </p>
                  <p className="text-xs text-foreground font-mono break-words">{initialProduct}</p>
                  <Link
                    href={sourcePath}
                    className="inline-block mt-3 text-xs font-semibold text-brand-700 hover:text-brand-800 transition-colors"
                  >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Pricing, MOQ, and lead-time turnaround within 1 business day.
                    </p>
                  </div>
                </div>
              </div>
              <div className="p-5 border border-border rounded-lg bg-muted/30">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Datasheets and compliance documentation can be attached in follow-up.
                    </p>
                  </div>
                </div>
              </div>
              <div className="p-5 border border-border rounded-lg bg-muted/30">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Each request is assigned a submission reference for tracking and support.
                    </p>
                  </div>
                </div>
              </div>
