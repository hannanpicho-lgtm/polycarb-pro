...
const CATEGORIES: { key: string; label: string; slugPrefix?: string[] }[] = [
  ...
  },
  {
    ...
    label: 'Specialty',
    slugPrefix: [
      'covestro-makrolon-ar',
      'sabic-lexan-fp',
      'trinseo-calibre',
      'lg-chem',
      'teijin',
      'mitsubishi',
      'covestro-makrolon-esd',
      'lexan-ballistic',
      'calibre-automotive',
      'lupoy-impact',
      'makrolon-medical',
...
function BuilderContent() {
  ...
  useEffect(() => {
    ...
    fetch('/api/catalog/prices')
      .then((r) => r.json())
      .then((d: { products?: PublicCatalogProduct[]; publicHiddenSlugs?: string[] }) => {
        if (cancelled) return;
        hydratePublicHiddenSlugsFromCatalogApi(d.publicHiddenSlugs);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (items.length === 0) {
      setSubmitError('Add at least one product to continue.');
      return;
    }
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-lg p-10 max-w-md w-full text-center">
          ...
          <p className="text-sm text-slate-500 mb-6">
            Our team will review your quote and respond within <strong>1 business day</strong>. Keep
            an eye on your inbox.
          </p>
          <div className="flex flex-col gap-2">
            <Link
              href="/products"
              className="block w-full py-2.5 bg-brand-600 text-white text-sm font-semibold rounded-lg hover:bg-brand-700 transition-colors"
            >
              Continue browsing products
            </Link>
            <Link
              href="/portal"
              className="block w-full py-2.5 border border-slate-200 text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-50 transition-colors"
            >
      ...
      <div className="bg-[#0a1628] pt-28 pb-10">
        <div className="container mx-auto">
          ...
          <p className="text-white/60 text-sm max-w-xl">
            Select products, set quantities, choose your shipping region — see live pricing
            instantly. Submit and our team responds within 1 business day.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="container mx-auto py-10">
        <div className="grid lg:grid-cols-5 gap-8 items-start">
                ...
                {catalogLoading ? (
                  ...
                  filteredProducts.map((p) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded flex-shrink-0 ${CAT_COLORS[cat] ?? ''}`}
                        >
                          {cat}
                        </span>
                        ...
                        <span className="text-xs text-slate-400 flex-shrink-0 font-mono">
                          {formatPrice(catalogListUnit(p, currency), currency)}/{p.unit}
                        </span>
                      </button>
                    );
                  })
                ...
                <div className="mt-4 flex items-center gap-3 bg-brand-50 border border-brand-200 rounded-xl px-4 py-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Min order: {selectedProduct.minQty} {selectedProduct.unit} · Lead time:{' '}
                      {selectedProduct.leadTimeDays}d
                    </p>
                  </div>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
                <div className="px-6 py-3 border-b border-slate-100 flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-400">
                    {items.length} product{items.length !== 1 ? 's' : ''} · {totalKg} kg total
                  </span>
                </div>
                      ...
                      <div key={item.id} className="flex items-center gap-4 px-6 py-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-400">
                            {formatPrice(unitPrice, currency)}/{item.product.unit}
                          </p>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <input
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={region?.id ?? ''}
                    onChange={(e) =>
                      setRegion(shippingRegions.find((r) => r.id === e.target.value) ?? null)
                    }
          ...
          <div className="lg:col-span-2 space-y-4 lg:sticky lg:top-24">
                  ...
                  <>
                    ...
                    <div className="border-t border-slate-100 pt-2.5 space-y-1.5">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold text-slate-800">
                          {region ? formatPrice(shipping, currency) : '—'}
                        </span>
                      </div>
            ...
            {/* Distributor program — high-visibility (retail quote vs partner pricing) */}
            <div className="rounded-2xl border-2 border-amber-300/60 bg-gradient-to-br from-amber-50 via-white to-amber-50/30 p-5 shadow-sm ring-1 ring-amber-200/50">
              <p className="text-[10px] font-bold uppercase tracking-widest text-amber-900/70">
                Wholesale &amp; OEM
              </p>
              <p className="text-sm font-bold text-slate-900 mt-1">Buying at scale or reselling?</p>
              <p className="text-xs text-slate-600 mt-1.5 leading-relaxed">
                This builder uses standard list pricing. For tiered distributor discounts, net
                terms, and portal access, apply to our partner program — reviewed within one
                business day.
              </p>
              <Link
                href="/distributors?source=quote-builder-sidebar"
                className="mt-4 flex w-full items-center justify-center rounded-xl bg-amber-500 px-4 py-3 text-sm font-bold text-white shadow-md shadow-amber-900/10 hover:bg-amber-600 transition-colors"
              >
                Apply for distributor pricing →
              </Link>
            </div>
            ...
            <div className="border-t border-slate-200 pt-4 space-y-2.5">
              ...
              >
                ...
                Already a distributor? Sign in to portal
              </Link>
            </div>
          </div>
        </div>
