...
export const metadata: Metadata = {
  title: 'Our Brands',
  description: 'Explore our partner polycarbonate brands including Covestro, SABIC, Trinseo, Teijin, Mitsubishi, and LG Chem.',
};
...
const categoryColors: Record<string, string> = {
  sheets: 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/30 dark:text-blue-400 dark:border-blue-900',
  rods: 'bg-slate-50 text-slate-700 border-slate-200 dark:bg-slate-950/30 dark:text-slate-400 dark:border-slate-800',
  resins: 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-950/30 dark:text-purple-400 dark:border-purple-900',
  specialty: 'bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-950/30 dark:text-orange-400 dark:border-orange-900',
};
    ...
    <>
      <div className="bg-gradient-to-b from-steel-950 via-steel-950 to-slate-900 pt-28 pb-16">
        <div className="container mx-auto">
          <p className="text-brand-400 text-[11px] font-bold uppercase tracking-[0.15em] mb-2">Authorised Distributor Network</p>
          ...
          <p className="text-white/70 text-base max-w-3xl leading-relaxed">
            We partner directly with the world's leading high-performance polymer producers. Each brand brings decades of material science excellence, vertical integration, and uncompromising quality standards. Your application deserves the best.
          </p>
        </div>
      </div>
        ...
        <div className="container mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {brandStats.map(({ brand, productCount, categories, inStockCount }) => {
              ...
              const colors = (brand.id as BrandKey) in brandColors 
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-br ${colors.bg} opacity-60 group-hover:opacity-100 transition-opacity duration-300 -z-10`} />
                  
                  {/* Accent top bar */}
                  <div className={`h-1 bg-gradient-to-r ${colors.accent} opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
                  
                  <div className="p-6 flex flex-col h-full">
                    {/* Logo and Header */}
                    <div className="flex items-start gap-4 mb-4">
                      {/* Logo container with brand-specific glow */}
                      <div className={`w-16 h-16 rounded-xl flex-shrink-0 overflow-hidden bg-white/80 dark:bg-slate-900 border border-border flex items-center justify-center ${colors.logoGlow} group-hover:scale-105 transition-transform duration-300`}>
                        {brand.logo ? (
                          <Image
                            src={brand.logo}
                            alt={brand.name}
                      ...
                      <div className="flex-1 min-w-0">
                        <h2 className={`font-bold text-sm text-foreground truncate ${colors.accent}`}>
                          {brand.name}
                        </h2>
                        <p className="text-xs text-muted-foreground mt-1 font-medium">{brand.country}</p>
                      </div>
                    </div>
                    ...
                    {productCount > 0 && (
                      <div className="inline-flex items-center gap-1.5 mb-4 w-fit">
                        <span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold ${colors.accent} bg-white/40 dark:bg-slate-800/40 backdrop-blur-sm border rounded-full px-2.5 py-1 whitespace-nowrap`}>
          ...
          <div className="mt-16 bg-gradient-to-r from-brand-600 to-brand-700 rounded-2xl p-10 text-center relative overflow-hidden">
            ...
            <p className="text-white/80 mb-6 max-w-2xl mx-auto">
              Need guidance on which brand and grade is right for your application? Our technical team combines material science expertise with decades of industry experience.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <Button asChild className="bg-white text-brand-600 hover:bg-slate-100 font-semibold px-8 h-11">
